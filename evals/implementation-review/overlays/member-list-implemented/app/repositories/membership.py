...
def list_active_members(org_id: int) -> list[Membership]:
    members = [m for m in _MEMBERSHIPS if m.org_id == org_id and m.status == "active"]
    return sorted(members, key=lambda m: (m.joined_at is None, m.joined_at))


def list_pending_invitations(org_id: int) -> list[Invitation]:
    return [i for i in _INVITATIONS if i.org_id == org_id and i.status == "pending"]
